...
exports.updateReportStats = updateReportStats;
exports.generatePackageReports = generatePackageReports;
 ...
 */
async function generateReports(report, outputDir) {
    // Ensure output directory exists
    await promises_1.default.mkdir(outputDir, { recursive: true });
    // Generate JSON report
    const jsonPath = path_1.default.join(outputDir, 'report.json');
    await promises_1.default.writeFile(jsonPath, JSON.stringify(report, null, 2), 'utf-8');
    console.log(`\n📄 JSON report saved: ${jsonPath}`);
    // Generate Markdown report
    const mdPath = path_1.default.join(outputDir, 'report.md');
...
function generateMarkdownReport(report) {
    ...
    lines.push(`**Duration:** ${(0, utils_1.formatDuration)(report.durationMs)}`);
    lines.push('');
    // Summary Statistics
    lines.push('## Summary');
    lines.push('');
 ...
 */
function generateBookingSection(booking, index) {
    const lines = [];
    ...
    for (const flow of booking.flows) {
        ...
        lines.push(`*Exploration time: ${(0, utils_1.formatDuration)(flow.explorationTimeMs)}*`);
        lines.push('');
    }
    lines.push('---');
    lines.push('');
 ...
 */
function formatGroupSizeRange(booking) {
    if (!booking.groupSizeConfig) {
 ...
 */
function collectAllAddOns(bookings) {
    const result = [];
    const seen = new Set();
 ...
 */
function createEmptyReport(baseUrl, config) {
 ...
 */
async function generatePackageReports(discoveries, outputDir) {
    await promises_1.default.mkdir(outputDir, { recursive: true });
    // Generate JSON report
    const jsonPath = path_1.default.join(outputDir, 'packages.json');
    await promises_1.default.writeFile(jsonPath, JSON.stringify(discoveries, null, 2), 'utf-8');
    console.log(`\n📦 Package JSON saved: ${jsonPath}`);
    // Generate Markdown report
    const mdPath = path_1.default.join(outputDir, 'PACKAGES.md');
 ...
 */
function generatePackageMarkdownReport(discoveries) {
    const lines = [];
    // Header
    ...
    for (const discovery of discoveries) {
        ...
        lines.push(`**Scrape Time:** ${(0, utils_1.formatDuration)(discovery.scrapeDurationMs)}`);
        lines.push('');
        // Categories
 ...
 */
function generatePackageSection(pkg) {
    const lines = [];
    lines.push(`##### 📦 ${pkg.name}`);
    lines.push('');
 ...
 */
function groupInclusionsByCategory(inclusions) {
    const byCategory = new Map();
 ...
 */
function getInclusionEmoji(category) {
    const emojis = {
        activity: '🎳',
        food: '🍕',
        drink: '🍺',
 ...
 */
function formatInclusionCategory(category) {
    const names = {
        activity: 'Activities',
        food: 'Food',
        drink: 'Drinks',
