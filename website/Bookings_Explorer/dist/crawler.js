...
class Crawler {
     ...
     */
    async crawl() {
        const formatLimit = (val) => val === Infinity ? 'unlimited' : String(val);
        console.log('\n🔍 Starting site crawl...');
            ...
            if (result) {
                ...
                for (const link of result.internalLinks) {
                    this.enqueue({
                        ...
                        priority: (0, utils_1.isBookingRelatedUrl)(link) ? item.depth : item.depth + 10,
                    });
                }
                // Collect triggers
     ...
     */
    enqueue(item) {
        const normalizedUrl = (0, utils_1.normalizeUrl)(item.url, this.config.baseUrl);
        // Skip external URLs
        if (!(0, utils_1.isInternalUrl)(normalizedUrl, this.config.baseUrl)) {
            return;
        }
    ...
    async visitPage(item) {
        ...
        const result = {
            ...
            internalLinks: [],
            bookingTriggers: [],
            visitedAt: (0, utils_1.getTimestamp)(),
            errors: [],
        };
        try {
            // Navigate to page
            ...
            result.bookingTriggers = await (0, detectors_1.detectBookingTriggers)(this.page, url);
        }
        catch (error) {
        ...
        try {
            ...
            for (const anchor of anchors) {
                try {
                    ...
                    if (href) {
                        const normalizedUrl = (0, utils_1.normalizeUrl)(href, this.config.baseUrl);
                        if ((0, utils_1.isInternalUrl)(normalizedUrl, this.config.baseUrl)) {
            ...
            const clickables = await this.page.locator('button[onclick*="location"], [data-href], [data-url], [data-link]').all();
            for (const elem of clickables) {
                try {
                    ...
                    if (dataHref) {
                        const normalizedUrl = (0, utils_1.normalizeUrl)(dataHref, this.config.baseUrl);
                        if ((0, utils_1.isInternalUrl)(normalizedUrl, this.config.baseUrl)) {
                            links.push(normalizedUrl);
                        }
                    }
 ...
 */
async function quickCrawl(page, config) {
    const crawler = new Crawler(page, config);
