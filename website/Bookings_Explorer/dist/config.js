...
function loadConfig() {
    ...
    const parseNum = (val, envVar, defaultVal) => {
        if (val !== undefined)
            return val;
        const envNum = parseInt(process.env[envVar] || '');
        if (!isNaN(envNum) && envNum > 0)
            return envNum;
        return defaultVal;
    };
    ...
    if (!path_1.default.isAbsolute(config.outputDir)) {
        config.outputDir = path_1.default.resolve(process.cwd(), config.outputDir);
    }
    // Validate URL
    try {
    ...
    config.outputDir = path_1.default.join(config.outputDir, domain);
    return config;
}
/**
...
function printConfig(config) {
    const formatLimit = (val) => val === Infinity || val === 0 ? 'unlimited' : String(val);
    console.log('\n📋 Configuration:');
