...
import { cleanText, extractPrice } from './utils';
 ...
 */
export async function extractGroupSizeConfig(page: Page): Promise<GroupSizeConfig | null> {
  const config: GroupSizeConfig = {
    minimum: null,
    maximum: null,
    ...
    try {
      ...
      if (await stepper.isVisible()) {
        ...
        if (await valueDisplay.isVisible()) {
          const value = await valueDisplay.inputValue().catch(() => 
            valueDisplay.textContent().then(t => t?.trim() || null)
          );
  ...
  if (config.availableOptions.length > 0) {
    const numericOptions = config.availableOptions.filter(
 ...
 */
function parseGroupSizeValue(value: string): number | string | null {
 ...
 */
async function extractCategoryFromLabel(input: Locator, page: Page): Promise<string | null> {
  try {
 ...
 */
export async function extractAvailableDates(page: Page): Promise<string[]> {
  const dates: string[] = [];
 ...
 */
export async function extractAvailableTimes(page: Page): Promise<string[]> {
  const times: string[] = [];
 ...
 */
export async function extractAddOns(page: Page): Promise<AddOn[]> {
  const addOns: AddOn[] = [];
  ...
  for (const containerSelector of containerSelectors) {
    try {
      ...
      if (await container.isVisible()) {
        ...
        const items = await container.locator(
          '.addon, .extra, .option, .item, [data-addon], label, .card'
        ).all();
 ...
 */
async function extractSingleAddOn(element: Locator): Promise<AddOn | null> {
  try {
    // Get text content
    ...
    let name = '';
    try {
      name = cleanText(await nameElement.textContent());
    } catch {
 ...
 */
async function extractAddOnFromCheckbox(
  checkbox: Locator,
  page: Page
): Promise<AddOn | null> {
  try {
    ...
    if (id) {
      const label = page.locator(`label[for="${id}"]`);
      labelText = cleanText(await label.textContent());
    }
    
    if (!labelText) {
      // Try parent
      const parent = checkbox.locator('xpath=..');
      labelText = cleanText(await parent.textContent());
    }
    
    if (!labelText || labelText.length < 3) return null;
    
    // Extract price from label
    const price = extractPrice(labelText);
 ...
 */
function categorizeAddOn(name: string, description: string): AddOnCategory {
 ...
 */
async function extractSingleFormField(
  input: Locator,
  page: Page
  ...
  try {
    ...
    else if (inputType === 'tel') type = 'phone';
    ...
    else if (inputType === 'time') type = 'time';
    else if (inputType === 'hidden') type = 'hidden';
    ...
    if (tagName === 'select') {
      ...
      for (const opt of optionElements) {
        ...
        if (text) options.push(cleanText(text));
      }
    }
    ...
    else selector = `${tagName}[type="${inputType}"]`;
 ...
 */
export async function extractPricing(page: Page): Promise<PricingInfo | null> {
    ...
    try {
      ...
      if (await element.isVisible()) {
        ...
        if (price) {
          ...
          // Detect currency
          if (price.includes('$')) pricing.currency = '$';
          else if (price.includes('€')) pricing.currency = '€';
          else if (price.includes('£')) pricing.currency = '£';
        }
      }
    } catch {
 ...
 */
export async function extractStepOptions(page: Page): Promise<StepOptions> {
  ...
  if (dates.length > 0) options.dates = dates;
  if (times.length > 0) options.times = times;
  if (pricing) options.pricing = pricing;
