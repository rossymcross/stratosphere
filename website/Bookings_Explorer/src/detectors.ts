...
import { cleanText, generateId, buildSelector, getElementInfo } from './utils';
 ...
 */
export async function detectBookingTriggers(
  page: Page,
  sourceUrl: string
): Promise<BookingTrigger[]> {
  const triggers: BookingTrigger[] = [];
...
async function analyzeElement(
  ...
  try {
    ...
    if (dataAttrs.some(attr => 
      DATA_ATTRIBUTE_PATTERNS.some(pattern => pattern.test(attr))
    )) {
    ...
    // Build selector
    const selector = await buildSelector(locator);
    ...
    let triggerType: BookingTrigger['triggerType'] = 'unknown';
    if (info.tagName === 'a') triggerType = 'link';
    else if (info.tagName === 'button' || info.tagName === 'input') triggerType = 'button';
    else if (info.tagName === 'form') triggerType = 'form';
 ...
 */
function deduplicateAndSort(triggers: BookingTrigger[]): BookingTrigger[] {
  const seen = new Set<string>();
  const unique: BookingTrigger[] = [];
 ...
 */
export async function detectBookingWidget(page: Page): Promise<boolean> {
 ...
 */
export async function isBookingLandingPage(page: Page): Promise<boolean> {
  // Check URL
 ...
 */
export async function detectLargeGroupIndicators(page: Page): Promise<{
